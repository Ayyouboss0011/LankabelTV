...
def get_latest_github_version():
    ...
    try:
        url = "https://api.github.com/repos/Ayyouboss0011/Aniworld-STO-Downloader/releases/latest"
...
def is_newest_version():
    ...
    try:
        ...
        latest_str = latest_tag.lstrip("v").lstrip(".")
...
DEFAULT_SERIES_PATH = os.getenv("SERIES_DOWNLOAD_PATH") or DEFAULT_DOWNLOAD_PATH
DEFAULT_MOVIE_PATH = os.getenv("MOVIE_DOWNLOAD_PATH") or DEFAULT_DOWNLOAD_PATH
...
DEFAULT_APPDATA_PATH = os.path.join(
    os.getenv("APPDATA") or os.path.expanduser("~"), "lankabeltv"
)
...
def _get_mpv_path():
    ...
    if _IS_WINDOWS and not mpv_path:
        mpv_path = os.path.join(os.getenv("APPDATA", ""), "lankabeltv", "mpv", "mpv.exe")
    return mpv_path
